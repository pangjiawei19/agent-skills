...
所有 Step 写作时引用本节。覆盖语言、标点、人味、mermaid、来源标注五个维度。

#### 4.1 语言与排版

- **默认语言：中文**。
- 中文文本中标点一律使用**全角**：`，。；：？！“”‘’（）【】《》——……`。
- **半角保留场景**：代码块、内联 code、文件路径、命令行、URL、邮箱、版本号、纯英文术语原文（如 `Node.js`、`v1.0.0`）。
- 中英文混排时**不加额外空格**，全角标点本身有视觉间距。
- **项目语言判定**：看 README 与主要代码注释的语言。如果 README 明显是英文为主，生成英文文档 + 半角标点；否则默认中文 + 全角标点。

#### 4.2 人味写作（产品视角段必遵）

- **一段叙述 ≤ 3 句**，白话能读完。
- **避免技术名词堆叠**：诸如“Aho-Corasick 多模匹配”“1536 维向量检索”“LanceDB”这类术语挪到“实现细节”区，产品视角段不出现。
- **结果导向、口语化表达**：写“翻过的句子被保存下来当参考”，不写“基于嵌入向量的语义检索”。
- **用具体场景而非泛泛能力**：“上线前批量过完整文案”比“批量翻译能力”有信息量。
- **不写调用链、不复述参数清单**：这些归实现细节区或域间链接。
- **判断标准**：产品人员能直接读完不卡壳的语气。

#### 4.3 Mermaid 语法约束

mermaid 渲染对特殊字符敏感，以下场景**必须用双引号包裹**：

| 场景 | 错误示例 | 正确示例 |
|------|---------|---------|
| 边标签含 `{ } [ ] ( ) \| /` | `A -->\|/api/{id}\| B` | `A -->\|"/api/{id}"\| B` |
| 节点文本含特殊字符 | `N[Agent (实时)]` | `N["Agent (实时)"]` |
| subgraph 标题含空格 / 中文 | `subgraph 服务进程 (Node)` | `subgraph proc["服务进程 (Node)"]` |
| 节点内换行 | 直接换行字符 | `<br/>` 或在双引号内用 `\n` |

#### 4.4 来源标注

- 重要事实标注来源：`Java 17（来源：pom.xml <java.version>）`。
- 行号引用频率适度，不必每段都堆，读起来累。
- 产品视角段**不标行号**，行号属于实现细节区。
...
### 混合区块：`auto:domains-table` 与 `auto:domains-listing`（行级既有权威）

两个区块共用同一种“混合区”语义——首次填入、之后既有行不重写、新增项追加，但内容来源略有不同：

| 区块 | 位置 | 首次填入数据来源 | 增量模式追加触发 |
|------|------|----------------|----------------|
| `auto:domains-table` | `01-architecture.md` 领域划分表 | Step 2 推断的域名 + paths + 核心实体 + 职责 | 代码出现未命中任何既有 path 的新路径 |
| `auto:domains-listing` | `domains/README.md` 领域清单表 | Step 2 推断的域名 + Step 2.4 子系统聚合的“提供什么能力” + 入口文档链接 | 代码出现新域时，新增一行（“提供什么能力”列由 AI 首次推断） |

**共同行为**：

- **首次生成**：按上表数据来源由 Step 7 推断填入。
- **增量模式**：
  - 每一**既有行**（包括单元格内容、备注、人工改过的域名 / 能力描述）视为**人工权威**，skill 永不覆写。
  - 出现新归属候选时**追加**新行，不动既有行。`auto:domains-table` 追加为“待归属：<path> · 建议：<域>”；`auto:domains-listing` 追加为完整的新域行。

这条规则让“用户手动调整过的划分”“手改的能力描述”“人工备注”全部保留，而 skill 仍能把新代码追加上来。

### 衍生 auto 块：`auto:domain-index`

`auto:domain-index` 展示域名列表，是 `auto:domains-table` 的衍生块。增量模式下按以下规则重算：
...
⚠️ 项目背景是 AI 最容易幻觉的部分。**不再用“待补充”占位**——首次生成时按下面优先级推断一段中性描述并写入文档，同时附 AI 推断标记：

*以下产品视角段落由 AI 首次生成，后续由维护者更新；skill 重跑不会覆写本节。*

数据源都不足时，降级为基于代码可观察事实的中性描述（如“本项目是一个多 Agent 服务，提供 X / Y / Z 三类能力，通过 REST 与 IM 对外暴露”），**仍然写一段**，不放占位符。准确性由人工修订保证。

**绝对不能从项目名瞎编业务定位。** 推不出就写中性事实描述，让人改；编出来的会误导。
...
输出精简版，详细版本留给 `03-dependencies.md`。找不到的类别**省略**，不写“无”。详细信号见 language-hints.md 的“架构组件识别”。
...
- 两者都没有：`02-milestones.md` **仍生成**，但“历史里程碑”整块写待补充
...
- **增量模式**：直接 `Read docs/wiki/01-architecture.md` 的 `auto:domains-table` 块作为归属依据，**不执行下面的 2.1 / 2.2**。只对代码里**未命中任何既有 path** 的孤儿路径做最小推断，输出一组“待归属”行（由 Step 7.5 追加到表尾）。2.3 单领域判断也不重跑——既有表已定性
...
### 2.2 输出到 01-architecture.md（首次模式）
...
### 2.4 子系统识别（供 Step 7 写产品视角段使用）

每个域的产品视角段需要列出“这个域有哪些子系统”。子系统粒度 = **Agent / Workflow / Job / 主要 Service 模块**这一层（即”对外能独立提供能力的运行单元”）。

**识别策略**（按项目类型，详见 [references/language-hints.md](references/language-hints.md) 各语言“子系统识别”小节）：

| 项目类型 | 子系统识别 |
|---------|-----------|
| Agent 框架（VoltAgent / LangGraph / 类似） | 每个 Agent 类 / Workflow 定义 / Job 注册各算一个子系统 |
| Spring Boot | `@Service` 标注的核心服务类（按职责聚合，不要拆得太细）；定时任务（`@Scheduled`）；MQ 消费者（`@KafkaListener` 等） |
| NestJS | 每个 `*.service.ts` 核心服务；定时任务（`@Cron`）；事件监听 |
| Express / Koa | 按路由组聚合（一组相关端点 = 一个子系统）；定时任务（`node-cron` 等） |
| Django / FastAPI | 按 router / blueprint 聚合；Celery `@task` 定时任务 |

**每个子系统记录三件事**（首次模式生成时由 AI 写入 domain README 产品视角段）：

1. **中文名**：从代码命名 + Agent instructions / 类注释推断（如 `TranslationAgent` → “批量翻译”，`alertAnalysisWorkflow` → “告警分析（Workflow 形态）”）
2. **能做什么 + 给谁用**：1 ~ 2 句白话。**遵守“写作规范”4.2 节的人味要求**——不堆技术名词，结果导向。
3. **入口**：REST 路径 / Cron 配置项名 / IM 路由关键词 / Workflow 注册名（按子系统类型给最具体的入口标识）

**数量上限**：每域子系统 ≤ 6 个。超出时合并相近的（例如术语库的 4 个 CRUD 端点合并成“术语库”一个子系统），或挪到末尾以列表形式补充次要子系统。

**与 Step 4 业务流程的关系**：Step 4 识别的是“端到端业务流程”（如下单流程）；Step 2.4 识别的是“运行单元”（如 OrderService、PaymentJob）。同一个子系统可以承载多个流程；同一个流程可以涉及多个子系统。两者不互相替代。
...
目的：让读 01-architecture.md 的人一眼看到目录、域、跨域实体关系三件事。不画全量 ER（全量在各域内画），避免这张图随实体增加而失控。
...
#### 7.4.1 domain README 写作流程（首次模式必遵）

每个 `docs/wiki/domains/<x>/README.md` 按以下骨架生成：
...
<2 ~ 4 句域叙事：业务痛点 + 域定位，遵守 4.2 节人味要求>
...
### 实现位置
[来源：Step 2 域 paths + 各子系统的实现文件]

### 对外接口
[来源：Step 4.1 业务入口 + 端点表]

### 核心实体
跳转 → [domain-model.md](./domain-model.md)（拆分模式）或保留实体节内容（单文件模式）

### 主要流程
跳转 → [flows.md](./flows.md)（拆分模式）或保留流程节内容（单文件模式）
...
1. **H1 升级**：`# <域 ID> · <域中文别名>`。中文别名由 AI 按域职责推断（例：`inspection · 多项目 K8s 巡检`），人工可改。
2. **顶部 AI 推断标记必须存在**：那行斜体提示首次生成必带；增量模式不动这行（用户可自行删除）。
3. **整段产品视角内容（H1、域叙事、子系统列表）不加 `auto` 标记**——归人工层。增量模式下完全不动（包括 H1）。
4. **`---` 分隔线 + `## 实现细节`** 是必需的，明确切开两类读者的视野。
5. 实现细节区内的 auto 块（实体跳转、流程跳转、`auto:endpoints` 等）行为照旧——属代码层，每次重算。

#### 7.4.2 项目层 README 业务背景写作流程（首次模式必遵）

`docs/wiki/README.md` 顶部“业务背景”小节按以下骨架生成：
...
<AI 推断的项目业务定位：解决什么问题、目标用户、核心价值。2 ~ 4 句白话>
```

推断数据源按 Step 1.4 列出的优先级：README 引言 / Agent instructions / 端点反推 / package.json description。都不足时降级写中性事实描述。**不放占位符。**

**不加 `auto` 标记**，归人工层。

#### 7.4.3 领域清单表（`auto:domains-listing`）的产品视角列

`docs/wiki/domains/README.md` 的领域清单表列从原来的 `| 域 | 一句话职责 | 核心实体 | 入口文档 |` 改为 `| 域 | 提供什么能力 | 入口文档 |`：

- “核心实体”列移除（对产品人员是噪音）。
- “提供什么能力”列内容用产品视角白话写（遵守 4.2 节人味要求）。

行为见 7.5（增量模式行为）和 [references/output-templates.md](references/output-templates.md)。
...
4. **处理代码里有但文档里没有的新项**：按模板生成新 auto 块（含标记和相邻的人工小节占位），追加到合理位置：
   ...
   - 新依赖类别 → 追加到 `03-dependencies.md` 对应小节
...
`04-deployment.md` 和 `05-development.md` 整体是人工层（大部分是团队规范 prose），未加 auto 标记。增量模式下这两个文件**完全不动**。首次模式按模板生成“待补充”占位，由用户填写后永久保留。
...
- 文档保存位置
- 手动调整建议（“如需修改领域划分，直接编辑 `01-architecture.md` 的 `auto:domains-table` 块”）
...
- `04-deployment.md` 和 `05-development.md` 未被修改（mtime 未变 或 内容 diff 为空）

### 8.7 产品视角与写作规范自检（首次模式必跑）

仅在首次模式跑（增量模式人工层不动，无须重检）：

- **mermaid 语法**：抽 1 ~ 2 个 mermaid 块，正则扫“边标签含 `{ } [ ] ( )` 但未用双引号包裹”。命中→修正后回写。如本地有 `mermaid-cli` 可执行（`which mmdc`），跑一次 `mmdc -i <tmp.mmd> -o /dev/null` 验证 parse 通过。
- **产品视角段不在 auto 块内**：扫每个 `domains/<x>/README.md` 顶部段落（H1 到第一个 `---` 之间）+ `docs/wiki/README.md` 的“业务背景”小节，确认这两段没有出现 `<!-- auto:xxx -->` 标记。命中→报告并修正（移除 AI 错误添加的 auto 标记）。
- **AI 推断标记到位**：产品视角段顶部必须有 `*以下产品视角段落由 AI 首次生成...*` 这行斜体提示。首次生成必须包含；增量不检查（人工可能删除）。
- **域 H1 含中文别名**：每个 `domains/<x>/README.md` 第一行格式为 `# <域 ID> · <域中文别名>`，必须含 `·` 分隔符。命中失败→补全别名（首次模式）。
- **全角标点**：扫所有生成的中文 markdown 文件（`docs/wiki/*.md`、`docs/wiki/domains/**/*.md`），用正则 `[一-龥][,;:?!][一-龥]` 找半角标点夹中文的情况。命中→报告位置，修正为全角。
