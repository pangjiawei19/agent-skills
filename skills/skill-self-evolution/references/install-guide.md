...
- 将修改目标 Skill 的哪些文件；
- 每个流程结束后如何写运行轨迹；
- 每个流程结束后如何轻量询问用户反馈，以及用户不回答时如何跳过；
- 何时触发阶段复盘；
- 哪些情况才允许修改目标 Skill；
...
3. 在目标主流程的收尾步骤后加入“轻量询问用户反馈”和“写入运行轨迹”。
...
- 运行结束写轨迹前默认轻问一句用户反馈；用户不回答则跳过，不反复追问。
- 用户反馈只能进入运行轨迹和候选改动，不能绕过阶段复盘和用户确认直接修改 Skill。
...
- runs/reviews 目录存在；
- 目标 Skill 能说明何时写运行轨迹；
- 目标 Skill 能说明何时询问用户反馈，以及用户不回答时如何跳过；
- 目标 Skill 能说明何时做阶段复盘；
