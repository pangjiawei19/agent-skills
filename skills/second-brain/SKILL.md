---
name: second-brain
description: 个人第二大脑 + 日常工作流：一天的开工/完成/收工 + 周总结 + 教练式对话/月度回顾/决策导航合一。Shortcuts：开工（列今日清单）、完成了 xxx（打勾）、收工（三段式复盘 + 询问反思）、周总结（编辑模式周报）、+chat（教练对话）、+navigate（决策导航分析，第一性原理 + 黄金圈）、+checkup（月度回顾）。用户提及"开工"、"完成了 xxx"、"搞定了 xxx"、"收工"、"下班"、"第二大脑"、"复盘一下"、"反思一下"、"我想倒一倒"、"开始反思"、"帮我分析"、"用框架捋一下"、"月度回顾"、"月度体检"、"周总结"、"周报"、"本周总结"、"上周总结"时触发。
---
...
│   ├── patterns-archive.md   # 已归档的旧 pattern（首次归档时由 +checkup 创建）
...
| Shortcut | 用途 | Reference |
|---|---|---|
| `周总结` / `周报` / `本周总结` / `上周总结` | 周期确认 → 汇总每日记录到三模块 → 提取关键词 → AI 视角整体评价 → 草稿确认 → 写入 reports/，末尾询问是否进入 `+chat` 反思 | [`references/weekly-summary.md`](references/weekly-summary.md) |
...
| `+navigate` / `帮我分析` / `用框架捋一下` | 决策导航分析（顾问模式）：第一性原理剥事实 → Why → What → How → 下一步动作 | [`references/navigate.md`](references/navigate.md) |
| `+checkup [--month YYYY-MM]` | 月度回顾报告（融合跨时间观察 + patterns 维护，patterns 维护降为 opt-in，健康告警由 agent 主动提） | [`references/checkup.md`](references/checkup.md) |
...
6. **人格切换规则**：
   ...
   - brain 段（`+navigate`）= **顾问模式**：按「第一性原理 + 黄金圈」框架带用户走结构化分析，反问 + 提炼候选，每步有结构化产出；**不点 patterns 归纳规律**（防止旧 pattern 污染新认知）
   - 切换点：**收工写入复盘文件后** agent 主动询问"要不要开始反思？"——用户显式说"开始反思"或"+chat"才切教练，否则保持秘书直到结束
   - 周总结切换点：写入 reports/ 后 agent 询问"要不要 `+chat` 开始反思？"——用户显式同意才切教练
7. **纪律优先级**（撞了按此排序）：
   ```
...
| `reports/YYYY-MM-checkup.md` | 自动写，不审阅 | brain |
...
| `profile/patterns-archive.md` | 用户同意归档/重写后自动追加（由 +checkup 触发）| brain |
