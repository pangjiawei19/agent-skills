...
AI 自动推断并直接写入文档——**不硬停等用户确认**。用户看到产出的"领域划分"表格后，不满意可以直接编辑 `architecture.md` 并重跑 skill。
...
### 2.2 输出到 architecture.md

把推断结果写入 `architecture.md` 的"领域划分"节，表格包含：**域名 / paths / 归属本域的核心实体 / 一句话职责**。用户想调整就改这张表 + 各域目录。
...
在 `architecture.md` 新增"领域划分"节，包含两部分：

1. **领域划分表**：Step 2 推断的结果（域名 / paths / 核心实体 / 一句话职责）
2. **跨域 ER 主图**（Mermaid `erDiagram`）：**只画涉及跨域引用的实体关系**（如 Order → User、Payment → User），域内实体细节留给各域 `domain-model.md`
...
按 [references/output-templates.md](references/output-templates.md) 的骨架，把 Step 1-6 产出分发：
...
给用户一段简短摘要：核心功能、技术栈、识别的业务域数量及名称、每域实体/流程数量、文档保存位置、手动调整建议（"如需修改领域划分，编辑 `architecture.md` 的'领域划分'表后重跑 skill"）。
