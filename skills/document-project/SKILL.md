...
每次调用 skill 前先 `Glob` 检查 `docs/project-overview/README.md`：
...
### 特殊区块：`auto:domains-table`

`architecture.md` 的领域划分表虽然标着 auto，但**处理方式不同**：

- 首次生成时由 Step 2 推断填入
- 增量模式下**以既有表为权威**：Step 2 读取后直接复用，不重新推断
- 代码里出现了未归属到任何 path 的新路径 → 在表尾追加一行"待归属：<path> · 建议：<域>"，让用户决定

这避免了用户手动调整过的划分被机械推断覆盖。
...
- **增量模式**：直接 `Read docs/project-overview/architecture.md` 的 `auto:domains-table` 块作为归属依据，**不执行下面的 2.1 / 2.2**。只对代码里**未命中任何既有 path** 的孤儿路径做最小推断，输出一组"待归属"行（由 Step 7.5 追加到表尾）。2.3 单领域判断也不重跑——既有表已定性
...
> 本项目识别为单领域，两层结构过重。建议直接写一份单页 README，或确认后我会为你生成单一 `docs/domain/` 目录。继续请回复"继续"。

用户要求继续则用退化结构（`docs/` 下直接 `README.md` + `domain-model.md` + `flows.md`）；否则跳过本 skill。
...
### 4.5 核心功能清单（供 Step 7 写入 project-overview/README.md）
...
**章节间互链**：
- `project-overview/README.md` 目录链接到本层其他文件 + `../domains/README.md`
...
**单领域退化**（Step 2.3 用户确认后）：`docs/` 下直接 `README.md` + `domain-model.md` + `flows.md`，不创建 `domains/`。

### 7.5 写入：增量模式（仅在 `docs/project-overview/` 已存在时）
...
4. **处理代码里有但文档里没有的新项**：按模板生成新 auto 块（含标记和相邻的人工小节占位），追加到合理位置：
   ...
   - 新核心功能 → 追加到 `project-overview/README.md` 的 `auto:features` 块内
...
- **不自动新建 `docs/domains/<new>/` 目录**——等用户确认归属后下次重跑 skill 再建
- 这避免了在归属未定时预先铺开文件结构
...
- `docs/project-overview/` 下 6 个文件都存在
- 多领域：`docs/domains/README.md` + 每个 `domains/<X>/README.md` 存在
- 单领域退化：`docs/` 下 `README.md` + `domain-model.md` + `flows.md` 存在

### 8.4 链接抽查

- `project-overview/README.md` 的目录链接指向的文件都存在
