...
skill 采用采样分析 + 并行子 agent 策略（SKILL.md "处理大型项目"小节）：每域核心实体 ≤ 10 个、每域主要流程 ≤ 5 个，其余用精简表格罗列。

### 增量更新是怎么判断的？

在 `project-overview/README.md` 头部写入 `<!-- DOC_META -->` 和 `<!-- DOMAIN_MAP -->` 锚点，下次运行时按 git diff 变更规模（<10% / 10-30% / >30%）决定增量或全量；增量时按 DOMAIN_MAP 分桶只更新受影响域。详见 SKILL.md Step 0。

### 领域是怎么划分的？

首次生成时按语言启发式推断并**硬停等用户确认**后写入 DOMAIN_MAP，之后增量更新以 DOMAIN_MAP 为准。如需调整，编辑 `project-overview/README.md` 头部的 DOMAIN_MAP 即可。详见 SKILL.md Step 1.5。
