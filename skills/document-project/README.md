...
- 重新运行 skill 会直接覆盖既有文档

### 如何更新文档？

直接重新运行 skill——会覆盖既有产出。如果只改了一两个文件，手动 diff 修改更快。

skill 不维护复杂的增量机制——当前实现倾向"一次性生成，手动维护"。

### 领域划分怎么调整？

1. 重新运行 skill 时附加说明，例如"把 `trade` 目录划到 order 域"
2. 或者直接编辑 `architecture.md` 的"领域划分"表 + 对应 `domains/<x>/` 目录
